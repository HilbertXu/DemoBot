...
class RobotHandDatasetSAPIENViewer(HandDatasetSAPIENViewer):
    ...
    def render_dexycb_data(self, data: Dict, fps=5, y_offset=0.8, render=False, data_dict=None):
        ...
        if data_dict is not None:
            for link in robot.get_links():
                if 'dummy' not in link.get_name():
                    data_dict[capture_name][link.get_name()] = []
            data_dict[capture_name]['qpos'] = []
        # Loop rendering
        ...
        for i in trange(start_frame, num_frame):
            ...
            for k in range(num_ycb_objects):
                ...
                # print(self.object_files[k].split('/')[-2], pose)
            ...
            for robot, retargeting, retarget2sapien in zip(self.robots, self.retargetings, self.retarget2sapien):
                ...
                robot.set_qpos(qpos)
                if data_dict is not None:
                    data_dict[capture_name]['qpos'].append(qpos)
                    
                joints = robot.get_active_joints()
                for j in joints:
                    print(j.get_name())
                # print('robot pos: ', robot.get_pose(), qpos.shape)
                # for link in robot.get_links():
                ...
                if data_dict is not None:
                    for link in robot.get_links():
                        if 'dummy' not in link.get_name():
                            ...
                            link_pos = link_pose.p
                            link_quat = link_pose.q
                            link_pos[1] += 0.8
                            data_dict[capture_name][link.get_name()].append(np.concatenate([np.asarray(link_pos), np.asarray(link_quat)]))
                        if link.get_name() == 'palm':
                            print(link.get_pose(), qpos[:6])
