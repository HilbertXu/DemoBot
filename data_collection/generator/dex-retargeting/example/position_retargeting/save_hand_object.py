...
def viz_hand_object(robots: Optional[Tuple[RobotName]], data_root: Path, fps: int):
    ...
    for data_id in pbar:
        ...
        for key, value in sampled_data.items():
            if "pose" not in key:
                print(f"{key}: {value}")
        pbar.set_description(f'{sampled_data["capture_name"]}')
        
        trajectory_dict[sampled_data['capture_name']] = {}
        
        viewer.load_object_hand(sampled_data)
        viewer.render_dexycb_data(sampled_data, fps, data_dict=trajectory_dict)
        
    with open('./dexycb_traj.pkl', 'wb') as f:
