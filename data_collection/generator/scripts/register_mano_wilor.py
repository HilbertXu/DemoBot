...
from src.hand_pose.registration import optimize_mano_shape_wilor
...
def fit_frame(
    ...
    params = optimize_mano_shape_wilor(
        target_v3d,
        mano_layers,
        optim_specs,
        iteration,
...
def fit_single_hand(v3d_ra_list, init_mano_params, data_dir, seq_name, args, is_right):
    import copy

    pbar = tqdm(enumerate(v3d_ra_list), total=len(v3d_ra_list))
    prev_out = {
        'betas': copy.deepcopy(init_mano_params['betas'][0]),
        'global_orient': matrix_to_axis_angle(copy.deepcopy(init_mano_params['global_orient'][0])),
        'hand_pose': matrix_to_axis_angle(copy.deepcopy(init_mano_params['hand_pose'][0])),
    }
    out_list = []
    for iteration, v3d_ra in pbar:
        ...
        if not is_valid:
            ...
            out = fit_frame(
                ...
                iteration=iteration,
                is_right=is_right,
                first_frame=iteration==0,
                use_beta_loss=args.use_beta_loss,
            )
            prev_out = {
                'betas': copy.deepcopy(init_mano_params['betas'][iteration]),
                'global_orient': matrix_to_axis_angle(copy.deepcopy(init_mano_params['global_orient'][iteration])),
                'hand_pose': matrix_to_axis_angle(copy.deepcopy(init_mano_params['hand_pose'][iteration])),
                'transl': out['transl']
            }
        out_list.append(out)
...
def main():
    ...
    print(data.keys())
    init_mano = np.load(f"{args.data_dir}/{seq_name}/processed/mano.init.npy", allow_pickle=True).item()
    init_mano = xdict(init_mano)
    ...
    for key, val in data.items():
        ...
        flag = key.split(".")[1]
        is_right = flag == "right"
        init_mano_params = init_mano.search(f'.{flag}')
        
        tmp_mano_params = {}
        for k,v in init_mano_params.items():
            tmp_mano_params[k.split(".")[0]] = torch.from_numpy(v).float().cuda()

        mydict = fit_single_hand(val, tmp_mano_params, data_dir, seq_name, args, is_right=is_right)
