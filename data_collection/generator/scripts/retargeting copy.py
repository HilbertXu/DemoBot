...
def parse_args():
    ...
    parser.add_argument("--robot", type=str, help="which robot model to be used for retargeting") 
...
class AllegroHandRetargeting(object):
    def __init__(
        ...
        self.robot_name = retarget_config.robot_name
        print(f"Retargeting from MANO hand to {self.robot_name}")
        ...
        self.retarget2sim = np.array([self.retargeting.joint_names.index(n) for n in sim_robot_joint_names]).astype(int)
        print(self.retarget2sim)
    ...
    def retarget(self, assets_root, scene_dir, camera_f):
        ...
        print(f"Save retargeted hand motions to {scene_dir}/retarget_{self.robot_name}.npz")
        np.savez(
            f'{scene_dir}/retarget_{self.robot_name}.npz', 
            qpos=results,
    ...
    def retarget(self, assets_root, scene_dir, camera_f, hand_type):
        # Load camera extrinsic matrix
        camera_pose = np.load(camera_f)
        extrinsic_matrix = np.eye(4)
        extrinsic_matrix[:3, :3] = camera_pose['R']
