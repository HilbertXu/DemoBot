...
def plot_2d_projection_object_mesh(pts_cano, o2w_all, intrinsic, seq_name):
    print("Projecting 3d points to 2d")

    pts_w = np.array(
        [np.dot(pts_cano, o2w_all[i].T) for i in range(o2w_all.shape[0])]
    )
    pts_w = pts_w[:, :, :3] / pts_w[:, :, 3:]
...
def format_poses(seq_name):
    ...
    print("Saving normalized point cloud to", pc_p, pts_cano.shape)
    print("Saving normalization matrix to", norm_mat_p, normalize_mat.shape)
    print("Saving intrinsic matrix to", intrinsic_p, intrinsic.shape, intrinsic)
    print("Saving pose matrix to", pose_p, o2w_all.shape)
...
def trim_object_mesh(sp_p, percentile=80, scale_factor=1.5):
    # this function trim point cloud by first computing its median
    # then find its 80 percentile for a threshold and pad with 1.5*thres to create a boundary
    # then we use the boundary to decide which points we include

    out_p = sp_p.replace(".obj", "trim.obj")
...
def read_object_pose(hwf_p):
    poses_hwf = np.load(hwf_p)
    poses_hwf = torch.FloatTensor(poses_hwf)

    poses_raw = poses_hwf[:, :3, :4]
...
def validate_object_pose(seq_name, no_vis, object_mesh):
    # read object poses and intrinsics
    print(f"./data/{seq_name}/*_pose_cam.npy")
    
    hwf_p = glob(f"./data/{seq_name}/*_pose_cam.npy")[0]
    o2w_all = read_object_pose(hwf_p)
    ...
    # remove outlier SfM points
    pc_trim = trim_object_mesh(
        object_mesh,
        percentile=80,
        scale_factor=1.5,
    )
    
    intrinsic = np.array([[602.36865234375, 0, 312.63818359375],
                          [0, 601.7484741210938, 245.52301025390625],
    ...
    # 2d projection
    if not no_vis:
        plot_2d_projection_object_mesh(
            np.array(pc_trim.vertices), o2w_all, intrinsic, seq_name
        )
