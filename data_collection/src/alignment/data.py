...
def read_data(data_dir, seq_name, K):
    ...
    colmap_pts = trimesh.load(
        f"{data_dir}/{seq_name}/processed/colmap/sparse_points_normalized.obj",
        process=False,
    ).vertices

    o2w_all = torch.FloatTensor(np.load(f"{data_dir}/{seq_name}/processed/colmap/o2w.npy"))

    # move in front of camera to avoid stucking at the back
    o2w_all[:, 2, 3] += 1.0

    normalize_mat = torch.FloatTensor(
        np.load(f"{data_dir}/{seq_name}/processed/colmap/normalization_mat.npy")
    )
    ...
    data = np.load(
        f"{data_dir}/{seq_name}/processed/hold_fit.slerp.npy", allow_pickle=True
    ).item()
    ...
    data_o["j2d.gt"] = torch.FloatTensor(
        np.load(f"{data_dir}/{seq_name}/processed/colmap_2d/keypoints.npy")
    )
